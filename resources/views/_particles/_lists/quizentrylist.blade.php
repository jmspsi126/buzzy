<input type="hidden" id="post_id" data-id="{{ $post->id }}">
        ...
        <div class="answer" style="margin-left:-15px;">
            ...
            <?php    $stringAnswer=''; ?>
                @foreach($sitep as $keya => $answers)
                    ...
                    if($stringAnswer!="")
                    $stringAnswer.='||'.$answers->id.'|'.$answers->is_correct;
                        else
                      $stringAnswer=$answers->id.'|'.$answers->is_correct;
                    ?>
                    <li>
                        <a class="@if(!empty($_COOKIE['post_' . $post->id]) && $_COOKIE['post_' . $post->id] == $answers->id) {{'active'}} @endif" href="javascript:"  data-answer="{{ $answers->id }}" data-result="{{ $answers->is_correct }}" onclick="checkResult('{{$answers->id}}','{{$answers->is_correct}}');" @if(empty($_COOKIE['post_' . $post->id])) onclick="checkResult('{{$answers->id}}','{{$answers->is_correct}}');" @endif id="href_{{ $answers->id }}">
                            <div class="answer-cover" >
                                @if($entry->video!='3')
                                    <img class="responsive-img" alt="{{ $answers->title }}" src="{{ makepreview($answers->image, null, 'answers') }}">
                                @endif
                                <h4 class="option-sel" >
                                    @if(empty($_COOKIE['post_' . $post->id]))
                                    <i class="fa fa-square-o answer-check"></i>
                                    @elseif(!empty($_COOKIE['post_' . $post->id]) && $_COOKIE['post_' . $post->id] == $answers->id)
                                    <i class="fa fa-square-o answer-check"></i>
                                    @endif
                    ...
                    @endif
                @endforeach
                <input type="hidden" id="answer_" value="{{$stringAnswer}}" />
              </ol>
        </div>
        <div class="clear"></div>
    </section>
...
<section class="entry results manage_social_link" id="quiz_result" data-popup="{{ getcong('BuzzyQuizzesPopup') }}">
    ...
    <!--<div class="external-sign-in">
     ...
     <div class="external-sign-in">
       ...
       <div class="addthis_sharing_toolbox"></div>
        ...
        {{--<a href="javascript:" class="Facebook postToResultFeed"></a>--}}
        {{--<a href="javascript:" class="Twitter postToResultFeed"></a>--}}
        {{--<a href="javascript:" class="Pinterest postToResultFeed"></a>--}}
        {{--<a href="https://plus.google.com/share?url={{ Request::url() }}" class="Google popup-action"></a>--}}
    </div>
    <div class="clear"></div>
    </div>
</section>
