                                ...
                                <a class="off" style="margin:0;padding:0" href="#ok">
                                    ...
                                    @else
                                        ...
                                        <a class="postable" style="margin:0;padding:0" data-method="Post" data-target="post-body" href="{{ action('PollController@VoteAPoll', [$post->type, $post->slug , 'vote' => $entry->id] ) }}" rel="nofollow">
                                            ...
                                            @endif
                                            {{--@endif--}}
                                            <div class="answer-cover" style="position: relative;">

                                                <h4 class="option-sel" style="z-index: 9999">
                                                ...
                                                @if(Auth::check() and $post->pollvotes()->currentUserHasVoteOnPost($post->id)->get()->first())
                                                 <!--   <div class="meta" style="font-size:26px;margin-top:-5px;font-weight: 700; z-index: 9999;">-->
                                                 <div class="meta" style="font-size:17px;margin-top:-5px;font-weight: 700; z-index: 9999;">
                                                    ...
                                                    <div class="result_bar" style="width: 50%; width: 100%;background-color: #ffffff; position: absolute; left: 0; top: 0; width: 100%; height: 100%; z-index: 0;">
                                                        <div class="poll_main_color result_bar_inner"  data-percent="{{$entry->vote()->count() ?  intval(($entry->vote()->count() / $post->pollvotes()->count()) * 100, 2) : 0 }}" style="width: 0%;background-color: #8ac5ff; height: inherit; transition: width .4s ease-in;-webkit-transition: width .4s ease-in;"></div>
                                                    </div>
