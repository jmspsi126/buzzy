  ...
  public function index(Posts $posts, User $users)
    {
        if (Auth::user()->usertype == 'Admin') {
            ...
            $postunapprove = $posts->approve('no')->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $todaypost = $posts->where('created_at', '>=', $rangetoday)->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $todayusers = $users->where('created_at', '>=', $rangetoday)->count();

            $todaylogins = $users->where('updated_at', '>=', $rangetoday)->count();

            $listcount = $posts->byType('list')->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $videocount = $posts->byType('video')->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $pollcount = $posts->byType('poll')->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $newscount = $posts->byType('news')->whereIn('domain_id', $Domains->lists('id')->toArray())->count();

            $lastunappruves =$posts->approve('no')->whereIn('domain_id', $Domains->lists('id')->toArray())->take('7')->latest("published_at")->get();

            $lastnews =$posts->approve('yes')->byType('news')->whereIn('domain_id', $Domains->lists('id')->toArray())->latest("published_at")->take('5')->get();
            $lastlists =$posts->approve('yes')->byType('list')->whereIn('domain_id', $Domains->lists('id')->toArray())->latest("published_at")->take('5')->get();
            $lastvideos = $posts->approve('yes')->byType('video')->whereIn('domain_id', $Domains->lists('id')->toArray())->latest("published_at")->take('5')->get();
            $lastpolls =$posts->approve('yes')->byType('poll')->whereIn('domain_id', $Domains->lists('id')->toArray())->latest("published_at")->take('5')->get();
            $lastquizzes =$posts->approve('yes')->byType('quiz')->whereIn('domain_id', $Domains->lists('id')->toArray())->latest("published_at")->take('5')->get();
    ...
    public function plugins()
    {
        ...
        if (!$plugins){
            \Session::flash('error.message', 'Plugins not available right now. Please try again later');
            return redirect('/admin');
        }

        $typeos=[];
        if(getcong('p-buzzynews') == 'on'){ $typeos['news']='News';
            $categories = Categories::byType("news")->get();
        ...
        if(getcong('p-buzzylists') == 'on'){ $typeos['list']='Lists';
        $categories = Categories::byType("list")->get();
        foreach($categories as $cat){
            $typeos[$cat->id]="---- " .$cat->name;
        }
        }
        if(getcong('p-buzzypolls') == 'on'){ $typeos['poll']='Polls';
        $categories = Categories::byType("poll")->get();
        foreach($categories as $cat){
            $typeos[$cat->id]="---- " .$cat->name;
        }
        }
        if(getcong('p-buzzyquizzes') == 'on'){ $typeos['quiz']='Quizzes';
        $categories = Categories::byType("quiz")->get();
        foreach($categories as $cat){
            $typeos[$cat->id]="---- " .$cat->name;
        }
        }
        if(getcong('p-buzzyvideos') == 'on'){ $typeos['video']='Videos';
        $categories = Categories::byType("video")->get();
        foreach($categories as $cat){
            $typeos[$cat->id]="---- " .$cat->name;
        }
        }
        ...
        if(\DbConfig::get('p-'.$pluginiten)=='on'){
            ...
            if($pluginverify=="on"){
                if(!$this->code($pluginiten)){
                    \Session::flash('error.message', 'We can not install this plugin. Please try to activate with your access code.');
    ...
    public function updatepurcahecheck(Request $request){
        ...
        if ($autoupload=='on'){
            ...
            if(!$this->getr($this->serverrar.$this->checkupdate().$this->sickupdate().'.zip')){
                $message['type']='error';
                $message['message']='Unable to get update files. Please try again later or try to get manuel download link.';
                $message['url']="";
            }
            $message['type']='success';
            $message['message']='Done. Your Buzzy System upgraded to Version '.$this->checkupdate();
            $message['url']="";
            return $message;
        }elseif ($content){
            $message['type']='success';
            $message['message']='Done. Downloading...';
            $message['url']=$this->serverrar.$this->checkupdate().$this->sickupdate().'.zip';
            return $message;
        }else{
            $message['type']='error';
            $message['message']='Your purchase code is not correct. Please Contact: contact@akbilisim.com';
            $message['url']="";
            return $message;
    ...
    public function checkinputcodeforplugin(Request $request){
        ...
        if (rop($jok['d'])){
            ...
            return response()->json(['type'=>'success', 'message' => 'Your Access Code is verified. You can install this plugin. ']);
        }elseif ($jok==null){
            return response()->json(['type'=>'error','message' => 'No record for your purchase code. Please Get New Access Code for this product from http://envato.akbilisim.com/.']);
        }elseif (!rop($jok['d'])){
            return response()->json(['type'=>'error','message' => 'This code already used by another domain. If you own this code please go http://envato.akbilisim.com/. And type correct domain name for this code.']);
        }else{
            return response()->json(['type'=>'error','message' => 'Your purchace is not correct. Please Get New Access Code  from http://envato.akbilisim.com/ and try again.']);
        }
